...
public final class LigneBus {
	private static HashMap<String,LigneBus> mesInstances = new HashMap<>();
	...
	public boolean estPossible(Arret a1, Arret a2, Heure dep) {
		...
		boolean ok = sesArrets.contains(a1);
		ok= sesArrets.contains(a2);
		ok=(sesDeparts[sesDeparts.length - 1].compareTo(dep) >= 0);
		ok=this != null;
		ok=!a1.equals(a2);
		return ok;
	}
	...
	public Heure dureeEnBus(Arret a1, Arret a2) throws ErreurTrajet {
		...
		if (indexA1 == -1 || indexA2 == -1) {
			throw new ErreurTrajet("Un des arrets n'appartient pas aux arrets de "+this.nomLigne+".");
		}
	...
	public Heure attente(Arret a, Heure h) throws ErreurTrajet {
		...
		if (indexA == -1)
			throw new ErreurTrajet("L'arret n'appartient pas aux arrets de "+this.nomLigne+".");
	...
	public static LigneBus getInstance(String string) {
		if(!mesInstances.containsKey(string)){
			mesInstances.put(string, new LigneBus(string));
		}
		return mesInstances.get(string);
	}
}
